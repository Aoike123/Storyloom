...
- 文字预审只记录、不返工：发现写入 `storyboard_review_notes` 并写审计记录，成片照常制作，制作页以「复核提示」展示给作者。它没有客观真值，因此不作为「必须修正」的指令回传模型，也不会自动让整部片重写；作者不同意时可以显式重试或重做当前节点。
- 重试与重做职责分离：重试只重跑失败产物并沿用原付款身份，重做删除本节点及之后节点的记录后重新开始；「继续」只续跑，不触发任何重跑。
