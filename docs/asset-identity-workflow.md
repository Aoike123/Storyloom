...
已废弃线路（`author-brainstorm-v1`–`v6`）：**定装合成**（人物身份图＋独立服装图合成定装）、**试拍**（定装＋场景组合试拍）与**镜头参考图**（逐镜生成图片再转视频）三个步骤已从线路中移除，代码与用例都改为注释保留，运行时不再加载。当前线路直接用已审核的人物身份图、独立服装图和场景图编写分镜，再据此生成视频。
