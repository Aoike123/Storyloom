...
代码规范：`asset-sheets-v4`。专业节点流程：`author-brainstorm-v7`（当前唯一在跑的线路）。节点来源和调用配置见 [专业 Skill 节点绑定](node-skill-bindings.md)。

已废弃线路（`author-brainstorm-v1`–`v6`）：**定装合成**（人物身份图＋独立服装图合成定装）、**试拍**（定装＋场景组合试拍）与**镜头参考图**（逐镜生成图片再转视频）三个步骤已从代码中删除，运行时只加载 v7；旧的 `/api/preproduction/{pid}/trial`、`/api/preproduction/{pid}/approve` 与 `/api/creative/{pid}/continue` 三个入口一并移除。当前线路直接用已审核的人物身份图、独立服装图和场景图编写分镜，再据此生成视频。
...
flowchart LR
  ...
  W --> C{该角色着衣物吗}
  C -->|着衣物 mode=garment| C1[独立服装图]
  C -->|天然体表 mode=bare| C2[空衣服结论：不生成服装图]
  W --> E[独立物理场景图]
  I --> R[作者确认基础素材]
  C1 --> R
  C2 --> R
...
### 角色 → 服装 → 环境是每个角色都要走完的一条链

服装节点对它锁定的每个 C 编号都必须给出答案，不能因为某个角色不穿衣服就跳过这一环。服装记录只有两种模式：

- `mode=garment`：实际服装，`wardrobe` 至少一件服饰部件，生成独立服装图。人类、类人以及身份节点标记为 required/optional 的角色一律使用它。
- `mode=bare`：空衣服模式，`wardrobe` 必须为空数组，`bare_surface` 写明自然体表与形体来自人物身份图、不添加任何衣物；**不生成服装图**，因为为天然体表生物画一张服装图等于凭空造衣物。

约束由代码保证：每个角色都必须至少有一条服装记录（缺少则 `AssetSheetPlan` 校验失败并把缺失的 C 编号回报给模型）、空衣服模式只允许用于 `costume_mode=none` **且没有人类身体分区**的角色、同一角色不能同时有空衣服模式与实际服装。因此"某个角色不穿衣"不再造成链路断档，也不会让人物身份图上的中性基础服装被误当成最终衣着。

身份图上给什么遮盖由**解剖分区**决定，不由整体构型名称或 `costume_mode` 决定：`兽首人身` 的颈部以下是人类躯干与四肢，所以它的身份图始终要求"人类躯干、臂手、腿足以无标识、低遮挡的中性基础短装覆盖，保留必要遮盖，不做裸露、紧身透视或性化呈现"，非人类分区的毛皮、鳞片、羽毛或甲壳同时完整露出。旧实现按 `body_plan` 搭配 `costume_mode` 判断，导致 `兽首人身` 在 `costume_mode=none` 时被要求"完整展示自然体表"，把人类躯干画成裸露并被供应商按违规内容拒绝（HTTP 451）。因此这类角色也不允许使用空衣服模式——空衣服模式等于要求一张裸体人类画面，必须在设计阶段就带着原因退回模型，让它给出实际服装。没有人类分区的主体（如覆盖毛皮的类人猿、龙、鸟、蛇）不受影响，照旧在自己不着衣物时展示自然体表。

空衣服结论会随基础素材保存（`costume_records` / `bare_costumes`），制作页在确认图片时显示"空衣服模式"清单；分镜契约里的 `bare_characters` 会告诉模型这些角色只绑定身份图与场景图，不要为它们选择服装或在画面里添加衣物。
...
HTTP 错误保存脱敏的供应商说明、错误码及请求编号，并按状态给出对应处理方式：402 核对余额或额度，429 等待后重试，451 说明"提示词或它要求的画面被判定违规"。451 是内容策略拒绝（例如要求裸露的自然体表），同样的提示词再提交一次仍会被拒绝，因此这类失败不是"重试这张图片"，而是把原提示词交回作者改写；面板会给出可编辑的提示词输入框，改写后重新提交，并在任务记录里标明这次提示词是手动改写的。其余可重试失败（余额、限流等）沿用已审核的原提示词，不允许在重试时顺手改词，避免画面偏离已确认的设计。网络中断、超时或 5xx 等结果仍不确定的调用不会通过这个入口再次提交，也不会自动重试。

在提交前就被拦下的图片请求同样进入这个入口，例如运营方 Key 当日被供应商暂停、算力豆不足或生图模型尚未配置。这些请求从未提交给供应商，重试不会重复计费，但也没有供应商响应可展示，界面会说明这一点并提示改用可用的额度；调用方可以在账号面板填写自己的 API Key 后逐张补齐。同一轮多张图片失败时，制作流程任务在最后一张图片重试后才排队，避免在其余图片仍未补齐时提前判定失败。

一次操作可以补齐整组失败图片。面板在多张图片失败时提供「全部重试这 N 张」，一次为每张图片排队新版本；单张按钮仍然保留，便于只补齐其中一张。

分镜与漫剧节点同理：一个节点包含它产出的全部画面和片段，节点暂停时会逐项列出停下的素材，而不是只报第一条错误。

## 三个动作，互不兼任

| 动作 | 入口 | 做什么 |
| --- | --- | --- |
| 继续 | `POST /projects/{pid}/resume` | 只用已保存的结果往下走。节点里还有停下的画面或片段时直接拒绝，并指向下面两个动作 |
| 重试 | `POST /projects/{pid}/retry-node` | 重跑这个节点失败的产物：分镜只重做没通过结构检查的片段，漫剧为每个停下的片段排队新版本；已经提交给供应商、只是结果未确认的片段沿用原任务编号重新核实，不重复提交 |
| 重做 | `POST /projects/{pid}/redo-node` | 删除这个节点及其后节点的全部分镜、片段、素材与任务记录（含文件与选用记录），把节点重置回入口状态后重新开始 |

三者曾经合并在一个函数里：「继续」也会重跑节点并重建素材，「重试」顺带作废子任务。现在重试只重跑、重做只重建，继续只续跑。

重跑时回传给模型的只有**代码可判定的结构错误**（镜号、原文引用、素材绑定、输出格式）。文字预审的意见属于创作判断，记录在制作页供作者复核，不再作为「必须逐条修正」的指令回传——那样会让分镜模型与评审反复争论，并让每次重试都重写整部片。

节点重排一律沿用原来的付款身份：work 侧自动恢复过去在请求作用域之外执行，排出的新节点没有付款身份，于是一开跑就报「请先用知乎账号登录领取算力豆」，即使浏览器已经填了自己的 Key。

## 制作动态的层级

制作动态按制作树展示：风格推荐、分镜生成、漫剧生成各占一行，它们派发的子任务折叠在该行之下；只有子任务需要处理时自动展开，让失败不会被折叠隐藏。
