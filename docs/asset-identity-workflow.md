...
约束由代码保证：每个角色都必须至少有一条服装记录（缺少则 `AssetSheetPlan` 校验失败并把缺失的 C 编号回报给模型）、空衣服模式只允许用于 `costume_mode=none` 的非人角色、同一角色不能同时有空衣服模式与实际服装。因此"某个角色不穿衣"不再造成链路断档，也不会让人物身份图上的中性基础服装被误当成最终衣着。
