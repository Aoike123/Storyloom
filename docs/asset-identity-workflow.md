...
镜头物理输入不超过三张时，原图保持分离且不会产生拼接图。只有完整绑定超过三张时，调用方才为该镜实际需要的身份—服装组合生成确定性的左右双栏参考板；它不调用 AI，不改变源图，并记录 `identity_asset_id`、`costume_asset_id` 与不可变依赖。拼接后仍超过三张时必须拆镜。
...
人物身份图、独立服装图、场景图直接作为镜头设计参考；仅在单镜超过三图限制时使用对应的本地双栏拼接板。镜头参考图用于确认构图与画风。视频请求冻结镜头参考图以及该镜实际绑定的参考图片，按固定顺序提交为 `reference_image`，运动由分镜 Prompt 描述。作者对视频提出修改时，`video_revision` 会把意见整合进完整视频 Prompt 并新建视频任务；旧视频不作为编辑输入。供应商图片数量与请求体限制在提交前检查。接口定义见 [MiniMax H3 参考视频模式](https://platform.minimax.cn/docs/api-reference/video-generation-v2-create)。
