...
export default function TaskActivity({task, detailed = false}: {task: ProgressTask; detailed?: boolean}) {
  ...
  const drafts = <StreamRegion active={active&&detailed} streamKey={streamKey+':drafts'} className={'activity-drafts'+(!activity.items?.length?' is-empty':'')} aria-label="逐步返回的创作内容"><div className="activity-draft-list">{activity.items?.map((item, index) => <article key={index}><strong>{item.title}</strong><p>{item.text ? <StreamText text={item.text} active={active && detailed && index === activity.items.length - 1} streamKey={streamKey + ':item:' + index} instant={problem || retired}/> : emptyDraft}</p></article>)}</div></StreamRegion>;
