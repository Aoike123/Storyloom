...
const stages:Record<string,string>={style:'先为这个故事，选择一种气质。',preparing:'故事中的人物，正在走向画面。',assets_review:'这些形象，符合你的想象吗？',storyboarding:'这段故事，拆成怎样的镜头？',rendering:'让分镜真正动起来。',film_review:'最后一次审片，让故事准备好登场。',published:'这段脑洞，已经有了画面。'};
export default function Author(){
 ...
 const reviewDegraded=creative?.storyboard_review_degraded;
   ...
   <div className="studio-layout">
    ...
    <div className="studio-main">
     ...
     <section className="author-panel studio-action"><div className="studio-action-heading"><span className="studio-step">{String(workStages.findIndex(s=>s.id===stage)+1).padStart(2,'0')}</span><div><span className="studio-eyebrow">{currentStage?.name||'制作进度'}</span><h2>{stages[stage||'']||'查看当前制作状态'}</h2></div></div>
      ...
      {!browsingEarlier&&<ImageRecovery images={work.retryable_images||[]} paid={paid} busy={busy||inFlight} onRetry={id=>act(async()=>{await api('/projects/'+selected+'/images/'+id+'/retry',{confirm_paid:true});setConfirmed(false);},'正在恢复这张失败图片，保留其他已完成素材…')} onRetryAll={()=>act(async()=>{await api('/projects/'+selected+'/images/retry',{confirm_paid:true});setConfirmed(false);},'正在一次恢复全部失败图片，保留其他已完成素材…')}/>}
      {['preparing','storyboarding','rendering'].includes(stage||'')&&<><p>{browsingEarlier?'正在回看本步骤的任务、提示词和已有结果。':currentStage?.hint+'。可以离开页面，后台会继续执行并保存进度。'}</p>{browsingEarlier&&!['storyboarding','rendering'].includes(stage||'')&&<button className="button primary" disabled={busy||inFlight||!paid} onClick={()=>act(async()=>{await restartPreparation();},'正在建立新的测试轮次，保留原有素材…')}>按当前风格重新测试制作</button>}{!browsingEarlier&&!work.retryable_images?.length&&['failed','needs_review'].includes(work.task?.status)&&<button className="button secondary" disabled={busy} onClick={()=>act(async()=>{await api('/projects/'+selected+'/resume',{});})}>{['storyboarding','rendering'].includes(stage||'')?'继续当前节点（复用已保存结果）':'问题处理后继续制作'}</button>}</>}
      ...
      {['storyboarding','rendering','film_review','published'].includes(stage||'')&&reviewDegraded&&<aside className="studio-review-handoff" role="status"><header><span><TriangleAlert size={18}/></span><div><small>FINAL FILM CHECK</small><h3>文字预审仍有分歧，已转为成片复核</h3></div></header><p>系统已按意见重做 {reviewDegraded.attempts||2} 次，没有继续空转；漫剧制作照常进行，请在审片时重点检查以下问题。</p>{reviewDegraded.issues?.length?<ul>{reviewDegraded.issues.map((issue:string,index:number)=><li key={index}>{issue}</li>)}</ul>:<p>预审没有给出具体问题，请完整检查镜头衔接与叙事节奏。</p>}<small>这不是自动通过。问题和本次降级决定均已保留在制作记录中。</small></aside>}
      {['storyboarding','rendering','film_review','published'].includes(stage||'')&&work.storyboard_review&&<details className={'studio-note studio-review '+(work.storyboard_review.approved?'is-passed':'is-rejected')} open={!work.storyboard_review.approved}><summary>分镜专业预审 · {work.storyboard_review.approved?'通过':reviewDegraded?'未通过，已转为成片复核':'未通过，已带着这些问题重做'}</summary><ul>{(work.storyboard_review.issues||[]).map((issue:string,index:number)=><li key={index}>{issue}</li>)}</ul>{!work.storyboard_review.issues?.length&&<p className="studio-note">预审没有给出具体问题。</p>}<p className="studio-note">这是文字预审，不是成片验收：连续性 {work.storyboard_review.continuity||'—'} · 戏剧逻辑 {work.storyboard_review.dramatic_logic||'—'} · 可剪辑性 {work.storyboard_review.editability||'—'} · 制作可行性 {work.storyboard_review.production_feasibility||'—'}</p></details>}
