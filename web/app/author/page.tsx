...
export default function Author(){
 ...
 const reviewNotes=work?.storyboard_review;
  // 发布一集不是这部作品的终点：还有没发布的情节时，发布后仍然要能继续做下一集。
  const remainingEpisodes=(work?.episodes||[]).filter((episode:any)=>!episode.published);
   ...
   {canRedoStage&&<section className={'studio-redo'+(redoCascades?' is-cascading':'')} aria-label="重做这一步">
     <div className="studio-redo-heading">
       ...
       <button type="button" className="button is-danger" disabled={busy||!paid}
         title={!paid?'请先勾选本页的模型调用许可':'删除这一步及其之后的成果，从这一步重新开始'}
   ...
   <div className="studio-layout">
    ...
    <div className="studio-main">
     ...
     <section className="author-panel studio-action"><div className="studio-action-heading"><span className="studio-step">{String(workStages.findIndex(s=>s.id===stage)+1).padStart(2,'0')}</span><div><span className="studio-eyebrow">{currentStage?.name||'制作进度'}</span><h2>{stages[stage||'']||'查看当前制作状态'}</h2></div></div>
      {/* 许可勾选框统一放在页面操作区，重做面板不再自带一个重复的。 */}
      {stage==='assets_review'&&<AssetRedesign paid={paid} busy={busy} running={inFlight} showPermission={false} onPaidChange={setPaid} onRedesign={()=>act(async()=>{if(browsingEarlier){await restartPreparation();}else{await api('/projects/'+selected+'/redesign',{art:work.art,tone:work.tone,confirm_paid:true});followProduction();}setConfirmed(false);},'正在按设定图规范重新设计人物与场景…')}/>}
